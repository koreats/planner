...
class GoalService {
  ...
  async getGoals(filters?: GoalFilters): Promise<Goal[]> {
    ...
    if (error) {
      ...
      throw new Error(error.message || '목표를 불러올 수 없습니다');
    }
  ...
  async getGoal(id: string): Promise<Goal> {
    ...
    if (error) {
      ...
      throw new Error(error.message || '목표를 찾을 수 없습니다');
    }
  ...
  async createGoal(input: CreateGoalInput): Promise<Goal> {
    console.log('[GoalService] createGoal input:', input);
    
    // 인증 확인
    const { data: { user }, error: authError } = await this.supabase.auth.getUser();
    
    if (authError) {
      console.error('[GoalService] Auth error:', authError);
      throw new Error('인증에 실패했습니다. 다시 로그인해주세요.');
    }
    
    if (!user) {
      console.error('[GoalService] No user found');
      throw new Error('로그인이 필요합니다. 다시 로그인해주세요.');
    }
    
    console.log('[GoalService] User authenticated:', user.id);
    ...
    console.log('[GoalService] Inserting goal data:', goalData);
    ...
    if (error) {
      console.error('[GoalService] Insert error:', error);
      ...
      if (error.code === '42501') {
        ...
        throw new Error('테이블이 존재하지 않습니다. 데이터베이스 마이그레이션을 확인해주세요.');
      } else if (error.code === '23505') {
        throw new Error('중복된 데이터입니다.');
      } else if (error.code === '23503') {
        throw new Error('참조 무결성 오류: 부모 목표를 찾을 수 없습니다.');
      } else {
        throw new Error(error.message || '목표를 생성할 수 없습니다');
      }
    }
  ...
  async updateGoal(input: UpdateGoalInput): Promise<Goal> {
    ...
    if (error) {
      ...
      throw new Error(error.message || '목표를 수정할 수 없습니다');
    }
  ...
  async deleteGoal(id: string): Promise<void> {
    ...
    if (error) {
      ...
      throw new Error(error.message || '목표를 삭제할 수 없습니다');
    }
  }
  ...
  async moveGoal(goalId: string, newParentId?: string): Promise<Goal> {
    ...
    if (error) {
      console.error('[GoalService] moveGoal error:', error);
      throw new Error(error.message || '목표를 이동할 수 없습니다');
    }
